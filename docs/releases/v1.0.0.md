...
- Exact command and host matching, explicit confirmation for shell composition
  and state-changing commands, and fail-closed invalid configuration.
- Dedicated gateway secret, constant-time comparison, bounded request/session
  state, and meaningful HTTP error statuses.
- Private randomized terminal output storage and byte-safe truncation.
...
- Functional local instruction skills selected with `--skill`, with private
  storage, path validation, and bounded loading.
